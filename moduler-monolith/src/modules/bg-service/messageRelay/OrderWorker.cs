...
namespace messageRelayService
{
    public class OrderWorker : BackgroundService
    {
        ...
        public OrderWorker(ILogger<OrderWorker> logger, IBus bus, IServiceProvider serviceProvider)
        {
            _logger = logger;
            _bus = bus;
        ...
        public async Task SendOutboxMessagesToBroker(CancellationToken stoppingToken)
        {
            ...
            using (IServiceScope scope = _serviceProvider.CreateScope())
            {
                ...
                var dbContextHandler = dbContextHandlers.Single(x => x.ContextId == config.GetValue<string>("Modules:Order:ContextId"));
